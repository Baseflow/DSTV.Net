...
namespace DSTV.Extensions
{
    ...
    internal static class TextReaderExtensions
    {
        ...
        internal static async Task<double> ParseDouble(this TextReader reader, ReaderContext context)
        {
            ...
            if (!double.TryParse(text, NumberStyles.Float, CultureInfo.InvariantCulture, out var value)) throw new DoubleParseException(context);
        ...
        /// <param name="context">The active reader context</param>
        /// <returns></returns>
        /// <exception cref="TupleParseException<double>"></exception>
        /// <exception cref="DoubleParseException"></exception>
            ...
            ReaderContext context)
        {
            ...
            if (!double.TryParse(split[0], NumberStyles.Float, CultureInfo.InvariantCulture, out var value1)) throw new DoubleParseException(context);

            context.IncrementLineNumber();

            if (split.Length == 2 && double.TryParse(split[1], NumberStyles.Float, CultureInfo.InvariantCulture, out var value2))
